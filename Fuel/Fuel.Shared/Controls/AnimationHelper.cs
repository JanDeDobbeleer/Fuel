...
namespace Fuel.Controls
{
    public class AnimationHelper
    {
        ...
        public static void AnimatePath(Windows.UI.Xaml.Shapes.Path progressPath, double radius, Point initialPoint, double finalAngle = 180, double timeStep = 0.01)
        {
            ...
            for (var i = 0; i <= finalAngle * multiplier; i++)
            {
                var discreteObjectKeyFrame = new DiscreteObjectKeyFrame
                {
                    KeyTime = KeyTime.FromTimeSpan(TimeSpan.FromSeconds(i*timeStep))
                };

                // create points for each ArcSegment
                Point firstArcPoint;
                Point secondArcPoint;
                ...
                const string dataValue = "m {0},{1} A {2},{2} 0 0 0 {3},{4} A {2},{2} 0 0 0 {5},{6}";
        ...
        {
            var animation = CreateHideOrDisplayElementAnimation(element, Visibility.Collapsed, 1, 0, TimeSpan.FromSeconds(duration));
            animation.Begin();
        }

        public static void DisplayElement(FrameworkElement element, double duration = 0.5)
        {
            var animation = CreateHideOrDisplayElementAnimation(element, Visibility.Visible, 0, 1, TimeSpan.FromSeconds(duration));
            animation.Begin();
        }
        ...
        private static Storyboard CreateHideOrDisplayElementAnimation(FrameworkElement element, Visibility visibility, double initialOpacity, double finalOpacity, TimeSpan duration)
        {
            ...
            {
                BeginTime = (visibility == Visibility.Visible) ? TimeSpan.FromSeconds(0) : duration
            };
            ...
            var opacityAnimation = new DoubleAnimation { From = initialOpacity, To = finalOpacity, Duration = new Duration(duration) };
